   ...
   private UpdateLog getAppUpdateLog(String appid, String currentVersion, Node appNode)
   {
     ...
     for (int i = 0; i < versionNodeList.getLength(); i++) {
       ...
       if (node.getNodeType() == 1) {
         ...
         if (versionLargerThen(version, currentVersion))
         {
           List<String> oneLogList = getVersionUpdateLog(appid, version);
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-eop.jar!\com\enation\app\base\core\service\impl\UpdateManager2.class
