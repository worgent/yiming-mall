 ...
 public class SiteMenuManager extends BaseSupport<SiteMenu> implements ISiteMenuManager
 {
   ...
   public List<SiteMenu> list(Integer parentid) {
     ...
     List<SiteMenu> topMenuList = new ArrayList();
     ...
     for (SiteMenu menu : menuList) {
       if (menu.getParentid().compareTo(parentid) == 0) {
         ...
         List<SiteMenu> children = getChildren(menuList, menu.getMenuid());

         int i = this.baseDaoSupport.queryForInt("select count(0) from es_site_menu where parentid=" + menu.getMenuid(), new Object[0]);
         if (i != 0) {
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-eop.jar!\com\enation\app\base\core\service\impl\SiteMenuManager.class
