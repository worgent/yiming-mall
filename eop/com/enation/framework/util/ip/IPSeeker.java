 ...
 public class IPSeeker
 {
     ...
     for (long offset = this.ipBegin + 4L; offset <= endOffset; offset += 7L)
     {
       ...
       if (temp != -1L) {
         ...
         if ((ipLoc.getCountry().indexOf(s) != -1) || (ipLoc.getArea().indexOf(s) != -1)) {
           ...
           entry.beginIp = IpUtil.getIpStringFromBytes(this.b4);

           readIP(temp, this.b4);
           entry.endIp = IpUtil.getIpStringFromBytes(this.b4);
   ...
   private long locateIP(byte[] ip)
   {
     ...
     long i = this.ipBegin; for (long j = this.ipEnd; i < j;) {
   ...
   private String readString(long offset)
   {
     try
     {
       ...
       int i = 0; for (this.buf[i] = this.ipFile.readByte(); this.buf[i] != 0; this.buf[(++i)] = this.ipFile.readByte()) {}
       if (i != 0)
   ...
   private String readString(int offset)
   {
     try
     {
       ...
       int i = 0; for (this.buf[i] = this.mbb.get(); this.buf[i] != 0; this.buf[(++i)] = this.mbb.get()) {}
       if (i != 0)
         return IpUtil.getString(this.buf, 0, i, "GBK");
   ...
   public static void main(String[] args) { IPSeeker ip = getInstance();
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-eop.jar!\com\enation\framework\\util\ip\IPSeeker.class
