   ...
   implements IMemberOrderManager
 {
   public Page pageOrders(int pageNo, int pageSize)
   {
     ...
     List<Map> list = (List)rpage.getResult();
     return rpage;
   }
   ...
   public Page pageOrders(int pageNo, int pageSize, String status, String keyword) {
     ...
     if (!StringUtil.isEmpty(keyword)) {
       sql = sql + " AND order_id in (SELECT i.order_id FROM " + getTableName("order_items") + " i LEFT JOIN " + getTableName("order") + " o ON i.order_id=o.order_id WHERE o.member_id='" + member.getMember_id() + "' AND i.name like '%" + keyword + "%')";
     }
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\MemberOrderManager.class
