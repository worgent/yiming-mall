 ...
 public class PrintTmplManager
   extends BaseSupport<PrintTmpl>
   implements IPrintTmplManager
 {
   public void add(PrintTmpl printTmpl)
   {
     ...
     newTmpl_data = replcePrintTmel(newTmpl_data, printTmpl.getBgimage());

     String code = this.baseDaoSupport.queryForString("select code from logi_company where name='" + printTmpl.getPrt_tmpl_title() + "'");

     printhtml(newTmpl_data, code, printTmpl);
   ...
   private void printhtml(String newTmpl_data, String code, PrintTmpl printTmpl) {
     File filename = new File(EopSetting.EOP_PATH + "\\shop\\admin\\printtpl\\express\\" + code + ".html");
     ...
     FileUtil.write(EopSetting.EOP_PATH + "\\shop\\admin\\printtpl\\express\\" + code + ".html", newTmpl_data);
   }



   private String newTmpl_data(String oldString, String replaceString, String newString)
   {
     String result = "";
     int i;
     do { i = oldString.indexOf(replaceString);
       if (i != -1) {
   ...
   public void clean(Integer[] id)
   {
     ...
     for (int i = 0; i < id.length; i++) {
       PrintTmpl printTmpl = get(id[i].intValue());
       String code = this.baseDaoSupport.queryForString("select code from logi_company where name='" + printTmpl.getPrt_tmpl_title() + "'");
       FileUtil.delete(EopSetting.EOP_PATH + "\\shop\\admin\\printtpl\\express\\" + code + ".html");
     }
   ...
   {
     PrintTmpl printTmpl = (PrintTmpl)this.baseDaoSupport.queryForObject("select * from print_tmpl where prt_tmpl_id = ?", PrintTmpl.class, new Object[] { Integer.valueOf(prt_tmpl_id) });
     String code = this.baseDaoSupport.queryForString("select code from logi_company where name='" + printTmpl.getPrt_tmpl_title() + "'");
     String lodopdata = FileUtil.read(EopSetting.EOP_PATH + "\\shop\\admin\\printtpl\\express\\" + code + ".html", "");
   ...
   private String replcePrintTmel(String newTmpl_data, String bgImage) {
     if (newTmpl_data.indexOf("src") != -1) {
       newTmpl_data = newTmpl_data.replace(newTmpl_data.substring(newTmpl_data.indexOf("src") + 5, newTmpl_data.indexOf(">") - 1), bgImage);
     }
   ...
   public String replcePrintTmelHTML(String newTmpl_data) { if (newTmpl_data.indexOf("${order.ship_no!''}") != -1) {
     ...
     if (newTmpl_data.indexOf("${order.weight!''}") != -1) {
       newTmpl_data = newTmpl_data(newTmpl_data, "${order.weight!''}", "订单物品总重量");
     }

     if (newTmpl_data.indexOf("${itemCount!''}") != -1) {
       newTmpl_data = newTmpl_data(newTmpl_data, "${itemCount!''}", "订单-物品数量");
     }
   ...
   public boolean check(String title) { if (this.daoSupport.queryForList("select * from es_print_tmpl where prt_tmpl_title=?", new Object[] { title }).size() > 0) {
       return true;
     }
     return false;
   }
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\PrintTmplManager.class
