   ...
   implements IBrandManager
 {
   ...
   public List groupByCat()
   {
     ...
     for (Map map : listCat) {
       ...
       List<Brand> listBrand = this.baseDaoSupport.queryForList("select * from brand where brand_id in (" + ids + ")", Brand.class, new Object[0]);
   ...
   {
     String sql = "select * from brand where disabled=0";
     Page page = this.baseDaoSupport.queryForPage(sql, 1, count, new BrandMapper(), new Object[0]);
     List list = (List)page.getResult();
     return list;
   }


   public Page searchBrand(Map brandMap, int page, int pageSize, String other)
   {
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\BrandManager.class
