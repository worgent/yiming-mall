 ...
 public class LimitBuyManager extends BaseSupport implements ILimitBuyManager
 {
   ...
   public List<LimitBuy> listEnable()
   {
     ...
     List<Map> goodsList = this.daoSupport.queryForList(sql, new Object[] { Long.valueOf(now), Long.valueOf(now) });
     for (LimitBuy limitBuy : limitBuyBuyList) {
       List<Map> list = findGoods(goodsList, limitBuy.getId());
       limitBuy.setGoodsList(list);
     }
     return limitBuyBuyList;
   }

   public List<Map> listEnableGoods() { long now = DateUtil.getDatelineLong();
   ...
   private List<Map> findGoods(List<Map> goodsList, Integer limitbuyid) { List<Map> list = new ArrayList();
   ...
   public LimitBuy get(Integer id) {
     ...
     List<Map> goodsList = this.daoSupport.queryForList(sql, new Object[] { id });
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\LimitBuyManager.class
