 ...
 public class DlyTypeManager
   extends BaseSupport<DlyType>
   implements IDlyTypeManager
 {
   ...
   public List<DlyType> list(Double weight, Double orderPrice, String regoinId)
   {
     ...
     List<DlyType> resultList = new ArrayList();
     ...
     for (DlyType dlyType : typeList)
     {
       ...
       if (dlyType.getIs_same().intValue() == 0) {
         List<TypeArea> areaList = filterTypeArea(dlyType.getType_id(), typeAreaList);
   ...
   {
     List<TypeArea> areaList = new ArrayList();
   ...
   public Integer add(DlyType type, DlyTypeConfig config)
   {
     ...
     return Integer.valueOf(this.baseDaoSupport.getLastId("dly_type"));
   }
   ...
   public Double[] countPrice(Integer typeId, Double weight, Double orderPrice, String regionId)
   {
     ...
     if ((dlyPrice == null) || (dlyPrice.doubleValue() == 0.0D)) {
       dlyPrice = countExp(dlyType.getExpressions(), weight, orderPrice);
     }
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\DlyTypeManager.class
