   ...
   implements IStatisticsManager
 {
   public List<MonthAmount> statisticsMonth_Amount()
   {
     ...
     List<MonthAmount> target = new ArrayList();
   ...
   public List<DayAmount> statisticsDay_Amount()
   {
     ...
     String sql = "";
     if (EopSetting.DBTYPE.equals("1")) {
       sql = "select sum(a.order_amount) as amount, Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m-%d') as mo from " + getTableName("order") + " a where a.status = " + 7 + " and Date_format(FROM_UNIXTIME(a.create_time / 1000),'%Y-%m') = ?  group by mo";
     } else if (EopSetting.DBTYPE.equals("2")) {
       sql = "select sum(a.order_amount) as amount, to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm-dd') as mo from " + getTableName("order") + " a where a.status = " + 7 + " and to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm') = ?  group by to_char(TO_DATE('01011970','mmddyyyy')+1/24/60/60*(a.create_time / 1000),'yyyy-mm-dd')";
     } else {
       sql = "select sum(a.order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10) as mo from " + getTableName("order") + " a where a.status = " + 7 + " and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ?  group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10)";
     }
     List<Map> list = this.daoSupport.queryForList(sql, new Object[] { year });
     List<DayAmount> target = new ArrayList();
   ...
   public List<DayAmount> statisticsDay_Amount(String monthinput)
   {
     ...
     if ("1".equals(EopSetting.DBTYPE)) {
       ...
       sql = "select sum(order_amount) as amount, substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10) as mo from " + getTableName("order") + " where status = " + 7 + " and substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,7) = ? group by substring(convert(varchar(10),dateadd(ss,create_time/1000 + 28800,'1970-01-01'),120),1,10)";
     }
   ...
   public static void main(String[] args)
   {
     ...
     String month; for (Iterator i$ = list.iterator(); i$.hasNext(); month = (String)i$.next()) {}
   }
 }


/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\StatisticsManager.class
