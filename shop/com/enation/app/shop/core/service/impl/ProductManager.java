   ...
   implements IProductManager
 {
   ...
   public List<Specification> listSpecs(Integer goodsId)
   {
     ...
     for (Map map : list) {
       ...
       if (spec_id.intValue() != dbspecid.intValue()) {
         ...
         spec.setSpec_type(Integer.valueOf(Integer.parseInt(map.get("spec_type").toString())));
   ...
   public List<Product> list(Integer goodsId)
   {
     ...
     List<Product> prolist = this.baseDaoSupport.queryForList(sql, Product.class, new Object[] { goodsId });

     sql = "select sv.*,gs.product_id product_id from  " + getTableName("goods_spec") + "  gs inner join " + getTableName("spec_values") + "  sv on gs.spec_value_id = sv.spec_value_id where  gs.goods_id=? order by gs.id desc";
     ...
     for (Product pro : prolist)
     {
       ...
       for (int i = size; i >= 0; i--) {
         ...
         if (pro.getProduct_id().intValue() == productid.intValue()) {
           ...
           spec.setSpec_type(Integer.valueOf(Integer.parseInt(gs.get("spec_type").toString())));
           pro.addSpec(spec);
         }
       }
     }
...
/* Location:              C:\Users\worgen\Desktop\临时\javashop_b2b2c.war!\WEB-INF\lib\component-shop.jar!\com\enation\app\shop\core\service\impl\ProductManager.class
