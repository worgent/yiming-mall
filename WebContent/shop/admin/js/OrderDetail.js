...
var OrderDetail = {
	...
	bindFlowEvent : function() {
		...
		$("#pay").bind("click",function() {
					...
					　　		closed: false,
					　　		cache: false,
					　　		href: 'payment!showPayDialog.do?ajax=yes&orderId='+ self.orderid, 	
					　　		modal: true,
					　　		buttons: [{		
					　　			 text:'保存',
					　　			 handler:function(){
		...
		$("#returned").bind("click",function() {
					...
					// $("#order_dialog
					// .con").load(basePath+"ship!showReturnDialog.do?ajax=yes&orderId="+self.orderid,function(){
					//				 
					// $("#order_dialog .submitBtn").unbind("click");
			...
			if( !disabled ){
				/*$.ajax({
		             type: "post",
		             url: "../shop/admin/sellBack!searchSn.do?orderId="+ self.orderid",
		             dataType: "json",
					...
					//location.href = basePath+"sellBack!add.do?orderId="+ self.orderid;
					newTab("申请退货","../shop/admin/sellBack!add.do?orderId="+ self.orderid);
			}
			});
		...
		$("#rog").bind("click",function() {
			...
			if( !disabled ){
					if (confirm("确认顾客已收到货了吗")) {
						...
						$.ajax({url : "order!rogConfirm.do?ajax=yes&orderId="+ self.orderid,
							dataType : "json",
	...
	ship:function(orderId){
		...
		$("#ship").linkbutton("disable");
		$.ajax({
			url :  'orderPrint!ship.do?ajax=yes&&order_id='+orderId,
			dataType : "json",
			cache:false,
			success : function(responseText) {
				if (responseText.result == 1) {
