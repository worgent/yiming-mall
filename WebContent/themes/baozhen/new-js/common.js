var SITE_URL = window.location.toString().split('/#')[0];
...
function login_dialog(){
	CUR_DIALOG = ajax_form('login','登录','#?act=login&inajax=1',360,1)
}
...
function collect_store(fav_id,jstype,jsobj){
	$.get('#?act=index&op=login', function(result){
	    if(result=='0'){
	    	login_dialog();
	    }else{
	    	var url = '#?act=member_favorites&op=favoritesstore';
...
function collect_goods(fav_id,jstype,jsobj){
	$.get('#?act=index&op=login', function(result){
	    if(result=='0'){
	    	...
	    	var url = '#?act=member_favorites&op=favoritesgoods';
	    	$.getJSON(url, {'fid':fav_id}, function(data){
	            if (data.done)
	            {
	                showDialog(data.msg, 'succ','','','','','','','','',2);
